...
joined as (
    ...
    left join {{ ref('dim_vendor') }}           dv
        on dv.payee_name = b.payee_name

    left join {{ ref('dim_expense_category') }} de
        on de.expense_category = b.expense_category

    left join {{ ref('dim_spending_category') }} ds
        on ds.spending_category = b.spending_category

    left join {{ ref('dim_mwbe_category') }}    dm
        on dm.mwbe_category = b.mwbe_category

    left join {{ ref('dim_date') }}             dd
        on dd.full_date = cast(b.issue_date as date)
)
